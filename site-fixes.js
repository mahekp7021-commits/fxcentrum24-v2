(() => {
  ...
  const ensureSocials = () => {
    document.querySelectorAll('.site-footer').forEach(footer => {
      if (footer.querySelector('.footer-social-standard')) return;
  ...
  const initPlatformTabs = () => {
    document.querySelectorAll('.platforms-section').forEach(section => {
      ...
      const select = platform => {
        ...
        tabs.forEach(tab=>{const active=tab.dataset.platform===platform;tab.classList.toggle('is-active',active);tab.setAttribute('aria-selected',active?'true':'false');});
        panels.forEach(panel=>{panel.hidden=panel.dataset.platformFeatures!==platform;});
        if(learn){learn.href=prefix()+item.path;learn.innerHTML=`${item.label} <b aria-hidden="true">→</b>`;}
      };
